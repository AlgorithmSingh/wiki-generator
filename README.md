...
> **Current readiness note (2026-06-24):** Phase 1-4 can produce a grounded baseline wiki with the Iteration 3 exact-coverage/public-route fixes and Phase 4 shell-variable path-synthesis fix. Strict sign-off is still blocked: the historical live wiki has a malformed evidence-like token and the output is too compact for the DeepWiki-informed coverage target. The active source of truth is `docs/specs/not-done/PHASE_DEEPWIKI_COVERAGE_ENHANCEMENT_ITERATION_SPEC.md`: Milestone 1 plus the Milestone 2 coverage taxonomy/validation, Phase 2 planning-obligation preservation, Phase 1 deterministic coverage-signal, Phase 2 planned-coverage gate, and the Phase 3 evidenced-coverage gate slices are implemented and tested. Phase 3 enhancement mode (`retrieve-evidence --coverage-mode enhancement`) maps required topics through explicit `topic_evidence_requirements[]` to exact source-field evidence IDs and fails the pipeline (exit 3, `bad_underspecified_normalized_plan`) on weak/missing required evidence before Phase 4. Phase 4 enhancement mode (`write-wiki --coverage-mode enhancement`) is now implemented (non-live): it refuses to call any provider unless the Phase 2 planned-coverage gate (`plans/coverage-gate.json`) and the Phase 3 evidenced-coverage gate (`evidence/evidenced-coverage.json` + the `required_topic_evidence_sufficient` contract check) are enforced/passing (else exit 3, pre-provider), preserves parent/child hierarchy in the index/metadata, carries the Phase 3 evidenced topic rows into WritingPackets, and deterministically validates that every evidenced sufficient required topic is generated with valid mapped citations (`generated_required_topics_covered` in `writing-validation.json`; `wiki/metadata/generated-coverage.json`; else exit 5). The non-live hierarchical E2E + benchmark-only comparison slice is also complete: the three phase shell wrappers now pass `--coverage-mode` (`tests/test_phase_wrappers.py`), and `scripts/nonlive_hierarchical_e2e.py` (no model calls) proved all three gates interoperate over an expanded 13-family hierarchical plan via the real CLI (planned 13/13, evidenced 13/13 sufficient, generated 13/13 covered, byte-identical on rerun, negative probe exit 3; `ragflow-deepwiki.md` compared as a structure-only benchmark). Pending next: explicit user approval before any live/billed retry. Do not run another live/billed retry until the user explicitly approves it.
...
# DeepWiki coverage enhancement (opt-in): refuse to call any provider unless the
# Phase 2 planned-coverage gate (normalize-plan --coverage-mode enhancement) and the
# Phase 3 evidenced-coverage gate (retrieve-evidence --coverage-mode enhancement)
# both passed (else exit 3, pre-provider). Preserves parent/child hierarchy and
# deterministically validates that every evidenced sufficient required topic is
# generated with valid mapped citations (else exit 5); writes
# wiki/metadata/generated-coverage.json + wiki/validation/generated-coverage-report.md.
wiki-generator write-wiki --bundle /path/to/phase1-output --provider vertex \
  --coverage-mode enhancement --project my-gcp-project --location us-central1
```
