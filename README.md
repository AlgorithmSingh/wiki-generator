...
> **Current readiness note (2026-06-26):** Phase 1-4 can produce a grounded baseline wiki, and the enhancement gates are implemented/tested non-live. The non-live hierarchical E2E passed over an expanded 13-family fixture. A user-approved live RAGFlow retry at `/Users/ankitsingh/Documents/deep-wiki/13-e2e-allphases/live-ragflow-enhancement-runs/20260625-141745` against `35bdc18` failed closed before Phase 3 because raw `evidence_needs.*` TER source-field aliases and old-readiness-only plan repair left topic obligations `0/46` complete. The follow-up non-live slice is now implemented at `41d422b`: Phase 2 normalization canonicalizes traceable raw TER aliases to `retrieval_needs.*`, leaves ambiguous/pruned aliases invalid instead of guessing, and `plan-repair --coverage-mode enhancement` accepts repair only when readiness, planned coverage, and topic-obligation gates all pass. Pending next: explicit user approval before any further live/billed retry.
