...
> **Current readiness status (2026-06-24):** Phase 1-4 can produce a grounded baseline wiki with the Iteration 3 exact-coverage/public-route fixes and the Phase 4 shell-variable path-synthesis fix. The successful live run is `/Users/ankitsingh/Documents/deep-wiki/13-e2e-allphases/phase4-live-vertex-runs/20260623-183730`, but strict sign-off is not complete: follow-up review found a malformed evidence-like token (`[ev:data-models:010]`) in the historical artifact and a broader coverage-target gap versus the DeepWiki benchmark. The active enhancement spec is `docs/specs/not-done/PHASE_DEEPWIKI_COVERAGE_ENHANCEMENT_ITERATION_SPEC.md`: Milestone 1 plus the Milestone 2 coverage taxonomy/validation, Phase 2 planning-obligation preservation, Phase 1 deterministic coverage-signal, Phase 2 planned-coverage gate, Phase 3 evidenced-coverage gate, and Phase 4 generated-coverage gate slices are implemented and tested non-live. Phase 3 enhancement mode (`retrieve-evidence --coverage-mode enhancement`) maps required topics through explicit `topic_evidence_requirements[]` to exact source-field evidence IDs and fails before Phase 4 on weak/missing required evidence. Phase 4 enhancement mode (`write-wiki --coverage-mode enhancement`) refuses provider calls unless planned/evidenced gates pass and validates generated required-topic coverage with mapped citations. The non-live hierarchical E2E + benchmark-only comparison slice is also complete: the three phase wrappers (`phase2_step2_normalize_plan.sh`, `phase3_retrieve_evidence.sh`, `phase4_write_wiki.sh`) now pass `--coverage-mode`, and `scripts/nonlive_hierarchical_e2e.py` (no model calls) proved all three gates interoperate over an expanded 13-family hierarchical plan via the real CLI. Pending next: explicit user approval before any live/billed retry. Do not run another live/billed retry until the user explicitly approves it.
