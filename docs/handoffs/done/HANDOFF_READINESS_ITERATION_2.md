# Handoff — Phase 1/2/3 Readiness Iteration 2 baseline and Phase 4 live-run follow-up

> **This remains the Iteration 2 readiness baseline handoff.** It supersedes `docs/handoffs/done/HANDOFF_READINESS_ITERATION.md` and `docs/handoffs/done/HANDOFF_STEP5.md` for the implemented Iteration 2 state. The next coding-agent source of truth is now `docs/specs/not-done/PHASE_DEEPWIKI_COVERAGE_ENHANCEMENT_ITERATION_SPEC.md`; do not modify `docs/specs/protected/PHASE3_EVIDENCE_RETRIEVAL_SPEC.md`.

Status: **Iteration 2 is implemented, tested, and accepted end-to-end on RAGFlow through Phase 3 without `--force`; the Iteration 3 exact-coverage/public-route fixes and the Phase 4 Iteration 2 shell-variable path-synthesis fix are also in the current Phase 3/4 baseline.** The accepted Phase 1-3 bundle is `/Users/ankitsingh/Documents/deep-wiki/13-e2e-allphases/runs/20260622-234038`. The successful live Phase 4 run is `/Users/ankitsingh/Documents/deep-wiki/13-e2e-allphases/phase4-live-vertex-runs/20260623-183730`, but strict sign-off is blocked by a malformed evidence-like token (`[ev:data-models:010]`) in the historical artifact and by the broader DeepWiki-informed coverage-enhancement gap. The malformed-token validator enhancement is implemented locally under `docs/specs/not-done/PHASE_DEEPWIKI_COVERAGE_ENHANCEMENT_ITERATION_SPEC.md` Milestone 1; Milestone 2 is not implemented.
...
Fresh run artifacts include `command-manifest.tsv`, `command-transcript.log`, exit codes, `validate_acceptance.py`, and `EXPERIMENT_RESULT.md`. Result: tests **175 passed / 1 skipped**, readiness **PASS** (0 failures, 0 warnings, 16 sections), Phase 3 without `--force` **PASS** (hybrid, 16/16, 569 evidence items, `failure_category: null`), 0 source evidence from `derived/` or `plans/`, 0 `no_retrieval_signal` sections, no normal `known-gaps` evidence, and `docs/specs/protected/PHASE3_EVIDENCE_RETRIEVAL_SPEC.md` unchanged. The latest successful live run at `/Users/ankitsingh/Documents/deep-wiki/13-e2e-allphases/phase4-live-vertex-runs/20260623-183730` refreshed deterministic Phase 3 successfully, verified citeable `rag/llm/embedding_model.py` and public-route evidence, generated 16/16 sections, and passed the then-current Phase 4 validation. Follow-up comparison found a malformed citation token and major coverage gaps. The malformed-token validator enhancement is now implemented locally; the next remaining step is Milestone 2 from `docs/specs/not-done/PHASE_DEEPWIKI_COVERAGE_ENHANCEMENT_ITERATION_SPEC.md`.
...
git diff --stat docs/specs/protected/PHASE3_EVIDENCE_RETRIEVAL_SPEC.md          # expect empty (unchanged)
...
- Phase 4 Iteration 2 (`docs/specs/done/PHASE4_WRITING_SYNTHESIS_ITERATION_2_SPEC.md`) is **implemented and validated non-live**. The prompt forbids shell/env-variable expansion; the validator classifies a deterministic one-step shell-variable expansion absent from evidence as a rewriteable `synthesized_identifier` (distinct from terminal `invented_identifier`) and feeds the rewrite the exact evidence tokens to use instead. It is rewriteable but never passable — final validation still requires exact tokens, and true inventions, routes, directory+filename joins, and multi-step/ambiguous derivations stay terminal. Proven by unit tests plus a fake-provider rewrite, re-checked against the real `20260623-172934` deployment artifacts, and exercised successfully in the live `20260623-183730` run. `docs/specs/protected/PHASE3_EVIDENCE_RETRIEVAL_SPEC.md` is unchanged. Before any further live Phase 4 retry, complete the non-live enhancement work in `docs/specs/not-done/PHASE_DEEPWIKI_COVERAGE_ENHANCEMENT_ITERATION_SPEC.md`, then obtain explicit user approval for another billed run; do not treat stale pre-patch artifacts, old `11-testing-pipeline` outputs, or any forced Phase-3-after-`FAIL` output as a Phase 4 GO.
