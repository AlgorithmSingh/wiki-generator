...
12. `specs/not-done/PHASE_DEEPWIKI_COVERAGE_ENHANCEMENT_ITERATION_SPEC.md` plus `handoffs/not-done/HANDOFF_DEEPWIKI_COVERAGE_ENHANCEMENT_ITERATION.md` — **active / not done**. Completed foundation slices: Milestone 1 malformed-token validation, Milestone 2 coverage taxonomy/validation, Phase 2 planning/PagePlan obligation preservation, Phase 1 deterministic coverage-signal expansion, Phase 2 planned-coverage upstream-prevention gating, Phase 3 evidenced coverage, Phase 4 enhancement-mode hierarchical writing + generated coverage, wrapper `--coverage-mode` support, the non-live hierarchical E2E + benchmark-only comparison, the Phase 2 required-topic evidence-obligation gate, and the Phase 2 TER source-field canonicalization + enhancement-mode bounded `plan-repair` diagnostics. A user-approved live RAGFlow retry at `/Users/ankitsingh/Documents/deep-wiki/13-e2e-allphases/live-ragflow-enhancement-runs/20260625-141745` failed closed before Phase 3: planned coverage passed 13/13, but topic obligations were `0/46` complete after bounded Step 1b repair. That failure is now addressed non-live: Phase 2 normalization canonicalizes raw `evidence_needs.*` TER source-field aliases to canonical `retrieval_needs.*` through a deterministic raw-index → normalized-index map (ambiguous/pruned aliases left invalid, never guessed), and `plan-repair --coverage-mode enhancement` accepts a repair only when readiness + planned-coverage + topic-obligation gates all pass, feeding the obligation diagnostics into the next attempt and failing loudly after the cap. Pending next: explicit user approval is required before any further billed retry.
