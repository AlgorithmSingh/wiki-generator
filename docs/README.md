...
12. `specs/not-done/PHASE_DEEPWIKI_COVERAGE_ENHANCEMENT_ITERATION_SPEC.md` plus `handoffs/not-done/HANDOFF_DEEPWIKI_COVERAGE_ENHANCEMENT_ITERATION.md` — **active / not done**. Completed foundation slices: Milestone 1 malformed-token validation, Milestone 2 coverage taxonomy/validation, Phase 2 planning/PagePlan obligation preservation, Phase 1 deterministic coverage-signal expansion, and Phase 2 planned-coverage upstream-prevention gating. Pending next: implement Phase 3 evidenced coverage using explicit `topic_evidence_requirements[]` mapped to exact source-field evidence IDs, where weak/missing required evidence is a pipeline failure before Phase 4 in enhancement mode; then Phase 4 hierarchical writing and generated coverage; non-live hierarchical E2E; then explicit approval before any billed live retry.
