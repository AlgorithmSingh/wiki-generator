...
12. `specs/not-done/PHASE_DEEPWIKI_COVERAGE_ENHANCEMENT_ITERATION_SPEC.md` plus `handoffs/not-done/HANDOFF_DEEPWIKI_COVERAGE_ENHANCEMENT_ITERATION.md` — **active / not done**. Completed foundation slices: Milestone 1 malformed-token validation, Milestone 2 coverage taxonomy/validation, Phase 2 planning/PagePlan obligation preservation, Phase 1 deterministic coverage-signal expansion, Phase 2 planned-coverage upstream-prevention gating, Phase 3 evidenced coverage (`retrieve-evidence --coverage-mode enhancement` maps required topics through explicit `topic_evidence_requirements[]` to exact source-field evidence IDs, failing the pipeline before Phase 4 on weak/missing required evidence), and Phase 4 enhancement-mode hierarchical writing + generated coverage (`write-wiki --coverage-mode enhancement` checks the Phase 2 planned-coverage and Phase 3 evidenced-coverage gates before any provider call (exit 3), preserves parent/child hierarchy, carries Phase 3 mapped evidence IDs into WritingPackets, and deterministically validates that every evidenced sufficient required topic is generated with valid mapped citations — `generated_required_topics_covered`, `wiki/metadata/generated-coverage.json`, exit 5 on failure). The non-live hierarchical E2E + benchmark-only comparison slice is also complete: the three phase shell wrappers were fixed to pass `--coverage-mode` (`tests/test_phase_wrappers.py`), and `scripts/nonlive_hierarchical_e2e.py` (no model calls) proved all three gates interoperate over an expanded 13-family hierarchical plan via the real CLI (planned 13/13, evidenced 13/13 sufficient, generated 13/13 covered, byte-identical on rerun, negative probe exit 3; `ragflow-deepwiki.md` compared as a structure-only benchmark). Pending next: explicit approval before any billed live retry.
